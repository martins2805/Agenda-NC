...
-- existente muda. Mantém a MESMA ordem de colunas corrigida na migration
-- 20260723120000 (origem_id e tipo_atividade_ids ao final, nunca no meio —
-- Postgres proíbe reordenar colunas de view via CREATE OR REPLACE). Idempotente
-- (lição da S1).
...
UNION ALL
  SELECT
    ...
    'registro', COALESCE(NULLIF(r."assunto", ''), r."nome"), r."empresaId", r."unidadeId",
