...
| 4 | "Execuções" (Cap. 2.3: "processos compostos por itens de execução... substituem checklists extensos") | Implementado como `model AtividadeGeral` + `ChecklistGeralItem`, rota `/atividades-gerais` — nomenclatura diferente da spec (`Execucao`/`ExecucaoItem`), mesmo conceito. Confirmado com o usuário antes de abrir a S10: é a mesma coisa, não um gap a fechar | Confirmado em 2026-07-23, ver `STATUS.md` "Dívidas assumidas" |
| 5 | Cap. 1.7 e 2.3 definem Execuções, Registros e Planilhas como módulos da primeira versão (entregues em S10–S12) | Módulos removidos da interface na S15 por pedido do usuário ("Adaptações Matheus", 2026-08-14). Dados e tabelas permanecem no banco, sem migration destrutiva — remoção reversível | D17 |
