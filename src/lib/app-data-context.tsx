...
import type { ConfiguracaoVisual } from "./visual-config";
...
interface AppDataContextValue {
  ...
  updateConfiguracoesVisuais: (list: ConfiguracaoVisual[]) => void;
  restaurarConfiguracaoVisualPadrao: () => void;
  loading: boolean;
...
export function AppDataProvider({ children }: { children: React.ReactNode }) {
  ...
  const [configuracoesVisuais, setConfiguracoesVisuais] = useState<ConfiguracaoVisual[]>([]);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (configuracaoVisualRes.ok) {
        const data = await configuracaoVisualRes.json();
        if (loadSeq.current !== seq) return;
        setConfiguracoesVisuais(data);
      }
    } catch (error) {
      if (loadSeq.current !== seq) return;
  ...
  // PROMPT 3 (3.1/3.2): mesma escrita otimista das preferências de widget.
  //
  // Envia só as chaves que mudaram. Mandar a lista inteira fazia a rota abrir
  // ~34 upserts numa transação só e estourar o timeout de 5s do Prisma contra
  // o banco remoto (P2028) — uma troca de cor custava 34 idas ao banco.
  const updateConfiguracoesVisuais = useCallback((list: ConfiguracaoVisual[]) => {
    setConfiguracoesVisuais((previous) => {
      const anterior = new Map(previous.map((c) => [c.chave, c]));
      const alteradas = list.filter((c) => {
        const a = anterior.get(c.chave);
        return (
          !a ||
          a.cor !== c.cor ||
          a.tamanho !== c.tamanho ||
      ...
      if (alteradas.length === 0) return previous;

      fetch("/api/configuracao-visual", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(alteradas),
      })
        .then((res) => {
          if (!res.ok) throw new Error(`PUT configuracao-visual falhou: ${res.status}`);
        })
        .catch((error) => {
          console.error("Falha ao salvar configuração visual", error);
          setConfiguracoesVisuais(previous);
          setDataError("Não foi possível salvar a configuração visual. A alteração foi desfeita.");
        });
      return list;
    });
  }, []);

  const restaurarConfiguracaoVisualPadrao = useCallback(() => {
    setConfiguracoesVisuais((previous) => {
      fetch("/api/configuracao-visual", { method: "DELETE" })
        .then(async (res) => {
          if (!res.ok) throw new Error(`DELETE configuracao-visual falhou: ${res.status}`);
          setConfiguracoesVisuais(await res.json());
        })
        .catch((error) => {
          console.error("Falha ao restaurar a configuração visual padrão", error);
          setConfiguracoesVisuais(previous);
          setDataError("Não foi possível restaurar o padrão. A alteração foi desfeita.");
        });
      // Otimista: cor/tamanho voltam ao padrão e tudo volta a ficar visível. A
      // ordem real vem na resposta do DELETE, que devolve o estado resolvido.
      return previous.map((c) => ({ ...c, cor: null, tamanho: null, visivel: true }));
    });
  }, []);
  ...
  const value = useMemo(
    () => ({
      ...
      restaurarConfiguracaoVisualPadrao,
      loading,
      dataError,
      dismissDataError,
      addLookupItem,
